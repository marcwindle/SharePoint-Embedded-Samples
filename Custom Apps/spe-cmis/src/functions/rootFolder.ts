...
/**
 * CMIS Browser Binding: Root Folder Operations
 * 
 * Handles all operations on the root folder of a repository.
 * Per CMIS 1.1 Browser Binding spec section 5.3.1.
 * 
 * URL Pattern: /storage/fileStorage/containerTypes/{containerTypeId}/cmis/browser/{repositoryId}/root
 * 
 * GET Operations (via cmisselector query param):
 *   - object: Get root folder properties
 *   - children: List children of root folder
 *   - content: Not applicable for folders
 * 
...
export async function rootFolder(
    ...
    context.log(`CMIS rootFolder request: ${request.method} ${request.url}`);
    ...
    if (request.method === 'GET') {
        return handleGet(request, context, graphAccessToken, repositoryId);
    } else if (request.method === 'POST') {
        return handlePost(request, context, graphAccessToken, repositoryId);
    }
...
/**
 * Handles GET requests on root folder
 */
async function handleGet(
    ...
    // CMIS Browser Binding id-based operations (used by clients for any object
    // that isn't the root, once its ID is known) are POSTed/GETed against the
    // repository's root folder URL with an `objectId` query/form parameter
    // rather than against a per-object URL. Default to 'root' when absent so
    // the root folder itself is still addressable without an objectId.
    const objectId = request.query.get('objectId') || 'root';

    context.log(`GET objectId=${objectId} cmisselector=${cmisselector}`);

    switch (cmisselector) {
        case 'object':
            return handleGetObject(accessToken, repositoryId, objectId, succinct, includeAllowableActions, includeACL);

        case 'children':
            return handleGetChildren(request, accessToken, repositoryId, objectId, succinct, includeAllowableActions);

        case 'content':
            return handleGetContent(accessToken, repositoryId, objectId);
...
/**
 * Gets object properties by objectId (or the root folder when objectId is 'root')
 */
async function handleGetObject(
    ...
    const result = objectId === 'root'
        ? await getDriveRoot(accessToken, repositoryId)
        : await getDriveItem(accessToken, repositoryId, objectId);
...
/**
 * Lists children of a folder identified by objectId (or the root folder)
 */
...
/**
 * Gets file content by objectId. Per CMIS Browser Binding, clients fetch
 * content for any object (not just root-level ones) via the root folder URL
 * with cmisselector=content and an objectId query parameter.
 */
...
/**
 * Handles POST requests on root folder
 */
async function handlePost(
    ...
    if (contentType.includes('multipart/form-data')) {
        ...
        context.log(`File: name=${fileName}, size=${fileContent?.length || 0}, mimeType=${mimeType}`);
    ...
    context.log(`POST cmisaction=${cmisaction}`);

    switch (cmisaction) {
        case 'createDocument':
            return handleCreateDocument(formData, fileContent, fileName, mimeType, accessToken, repositoryId);

        case 'createFolder':
            return handleCreateFolder(formData, accessToken, repositoryId);
        ...
        case 'setContent':
            return handleSetContentAction(formData, fileContent, mimeType, accessToken, repositoryId);
...
/**
 * Creates a new document (file) in the root folder
 */
async function handleCreateDocument(
    ...
    // The parent folder is identified by the `objectId` form field (per
    // cmislib), defaulting to the root folder when absent.
    const parentId = formData.get('objectId') || 'root';
...
app.http('rootFolder', {
    methods: ['GET', 'POST'],
    authLevel: 'anonymous',
    route: 'storage/fileStorage/containerTypes/{containerTypeId}/cmis/browser/{repositoryId}/root',
    handler: rootFolder
});
